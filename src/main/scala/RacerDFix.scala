...
object RacerDFix {
  ...
  def runPatchAndFix(config: FixConfig, iteration: Int): Int = {
    ...
    if(config.flag1) println("Reading config!")
    val jsonTranslator = new InterpretJson(config)
    if(config.flag1) println("End reading config!")
    if(config.flag1) println("Reading sumamries!")
    ...
    if(config.flag1) println("End reading sumamries!")
    ...
    if(config.flag1) println("End reading bugs json!")
