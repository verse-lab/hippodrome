...
import org.racerdfix.language.{RFSumm, SummaryIn}
...
object RacerDFix {
  ...
  def runPatchAndFix(config: FixConfig): Unit = {
    ...
    val options = Seq("--racerdfix", "--no-racerd", "--results-dir", Globals.results_out_dir)
    ...
    /* read the json files generated by infer and store the results into racerdfix's data structures*/
    val newBugsIn      = jsonTranslator.getJsonBugs()
    val newBugs        = newBugsIn.results.map(b => b.racerDToRacerDFix(summaries))
    if(newBugs.length > 0) {
